...
SUBJECT_DBS = ["OOP-152", "DDM-152", "DMA-152"]
EXCLUDED_ROLES = ["admin", "staff"]

CONNECTED_SCANNERS = []
SCANNER_HEARTBEATS = {}  # {scanner_id: datetime_last_seen}
STUDENT_HEARTBEATS = (
    {}
)  # {student_id: {"last_seen": datetime, "location": str}}
...
def get_scanner_info(client_id):
    """Assigns each connecting client a Scanner Rank and retrieves its DB-configured Module."""
    with scanner_lock:
        if client_id not in CONNECTED_SCANNERS:
            CONNECTED_SCANNERS.append(client_id)
        rank = CONNECTED_SCANNERS.index(client_id) + 1
        SCANNER_HEARTBEATS[rank] = datetime.now()

    with get_db("users.db") as conn:
        c = conn.cursor()
        c.execute(
            "SELECT module FROM scanner_config WHERE scanner_id = ?", (rank,)
        )
        row = c.fetchone()
        module = row[0] if row else f"MODULE-{rank}"
...
<head>
    ...
    <style>
        ...
        .links { margin-top: 15px; display: flex; justify-content: space-around; font-size: 13px; font-weight: bold; }
        .portal-link { color: #0066cc; text-decoration: none; }
    </style>
</head>
<body>
    <div class="card">
        ...
        <div class="links">
            <a href="/lecturer" class="portal-link">🔑 Lecturer Portal</a>
            <a href="/admin" class="portal-link">⚙️ Admin Control</a>
        </div>
    </div>
    ...
    <script>
        function updateInfo() {
            fetch('/api/scanner_info')
                .then(res => res.json())
                .then(info => {
                    document.getElementById('scannerBadge').innerText = `Scanner #${info.scanner_id} — Module: ${info.module}`;
                });
        }
        updateInfo();
        ...
        function processBarcode(code) {
            ...
            .then(data => {
                ...
                if(data.status === 'SUCCESS') {
                    ...
                    resDiv.innerHTML = `✅ <strong>Marked Present</strong><br>Name: ${data.name} (${data.role})<br>Module: ${data.module}`;
                } else {
