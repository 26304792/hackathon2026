...
def validate():
    ...
    with get_db("users.db") as conn:
        ...
        min_role = sc_config[0] if sc_config and sc_config[0] else "student"
        temp_role = sc_config[1] if sc_config and sc_config[1] else "none"
        temp_unlocked_until = sc_config[2] if sc_config else None
    ...
    if has_temp_gate and not is_unlocked:
    ...
    if user_lvl < req_lvl:
        return jsonify({
            "status": "REJECTED", 
            "message": f"Access Denied: Requires role '{required_role}' to access."
        })

    # Unlock scanner for 30 minutes if standard user meets temporary role requirement
    ...
    def auto_refresh():
        try:
            with sqlite3.connect("users.db", timeout=1.0) as conn:
                rows = conn.execute(
                    "SELECT student_number, name, permission_level, course FROM users"
                ).fetchall()
